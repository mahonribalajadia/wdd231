...
const displayMembers = (members) => {
    let filteredMembers = members.filter(member => member.level === 2 || member.level === 3).sort(() => 0.5 - Math.random()).slice(0, 3);
    ...
    filteredMembers.forEach((member) => {
        ...
        if (member.level == 2) {
            memberLvl.textContent = "Silver Member";
        } if (member.level == 3) {
            memberLvl.textContent = "Gold Member";
        }
        ...
        detailsWrapper.appendChild(memberLvl);
